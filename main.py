...
class KostApp(ctk.CTk):
    ...
    def setup_sidebar(self):
        ...
        ctk.CTkLabel(self.sidebar_frame, text="KOST ELITE\nManagement", font=ctk.CTkFont(size=22, weight="bold"), text_color="#4a90e2").grid(row=0, column=0, padx=20, pady=(30, 20))

        # Tombol Navigasi dengan Lambda
        ...
        ctk.CTkLabel(self.sidebar_frame, text="v1.0.0 Pro", text_color="gray").grid(row=6, column=0, pady=20)

    def create_btn(self, text, cmd, row):
        ctk.CTkButton(self.sidebar_frame, text=text, command=cmd, height=40, corner_radius=8, fg_color="transparent", hover_color="#3a3a3a", anchor="w", font=ctk.CTkFont(size=14)).grid(row=row, column=0, padx=15, pady=5, sticky="ew")
