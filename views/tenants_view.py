...
class TenantsView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(self, text="Manajemen Data Penghuni (Check-In)", font=ctk.CTkFont(size=24, weight="bold")).pack(anchor="w", pady=(0, 20))
        ...
        ctk.CTkLabel(input_frame, text="Pilih Kamar (Available):").grid(row=0, column=1, padx=20, pady=5, sticky="w")
    ...
    def load_table(self):
        ...
        for t in tenants:
            ...
            ctk.CTkButton(row, text=f"{t[6]}", width=100, height=25, fg_color="#4a90e2", hover=False).pack(side="left", padx=10)
    ...
    def save_tenant(self):
        try:
            if not self.entry_name.get() or not self.entry_phone.get():
                messagebox.showwarning("Warning", "Data harus lengkap!")
                return
            if not self.room_options:
                messagebox.showerror("Error", "Tidak ada kamar tersedia!")
                return
